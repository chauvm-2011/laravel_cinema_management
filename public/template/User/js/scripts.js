	...
	$('.entry-order-content').each(function(){
        ...
        $(".next1").on('click',function(){
            ...
            if (document.getElementById("selected-seats").innerHTML !== ''){
                ...
                Swal.fire({
                    title: 'Please choose your seats first.',
                    icon: 'warning',
                    showClass: {
                        popup: 'animate__animated animate__fadeInDown'
                    },
		...
		$(".previous").on('click',function(){
            ...
            document.getElementById("counter").innerHTML = number_seat
            document.getElementById("total").innerHTML = total
            document.getElementById("selected-seats").innerHTML = seat
            $('.selected').removeClass('selected')
            $('.seatCharts-seat').each(function (){
                $(this).removeAttr('aria-checked')
                $(this).addClass('available')
                $('.available').attr('aria-checked','false')
            })
...
var seat = [];
var status = [];
function myFunctionTime(time,id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // status.push(sc.seatIds)
            $(".next").on('click', function () {
                sc.get(sc.seatIds).status('available');
                $.each(data.tickets, function(key, item) {
                    var seat = item.seat_name
                    sc.get([seat]).status('unavailable');
                });
            })
            $(".submit").on('click',function(){
                var check = document.getElementById('vnpay');
