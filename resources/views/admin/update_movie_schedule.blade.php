                                    ...
                                    <select class=" form-control select2" name="showtime" data-placeholder="Select a start time" style="width: 100%;height: 50px">
                                        @foreach($movieshowtimes as $movieshowtime)
                                            @php
                                                $selected = false;
                                            @endphp
                                            @foreach($movie_schedule->movieshowtimes as $time)
                                                @if($movieshowtime->id === $time->id)
                                                    @php
                                                        $selected = true;
                                                    @endphp
                                                @endif
                                                @break
                                            @endforeach
                                            <option value="{{ $movieshowtime->id }}" {{ $selected ? 'selected' : '' }}>{{ $movieshowtime->start_time }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
